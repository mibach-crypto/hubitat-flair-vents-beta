...
private BigDecimal getRoomTemp(def vent) {
  ...
  // 1. If enabled, prefer latest cached raw sample
  ...
  // 4. All sources failed. Return null.
  log(2, 'App', "ERROR: No temperature available for room '${roomName}' from any source!")
  return null
}
...
def patchVent(device, percentOpen) {
  // The caller (initializeRoomStates or Quick Controls) is responsible for determining the correct value,
  // including respecting manual overrides via applyOverridesAndFloors or similar logic.
...
private BigDecimal getEwmaRate(String roomId, String hvacMode, Integer hour) {
  String hourStr = hour.toString() // Use string key
  try { return (atomicState?.dabEwma?.get(roomId)?.get(hvacMode)?.get(hourStr)) as BigDecimal } catch (ignore) { return null }
}
        ...
        try {
          ...
          hist.entries.each { e ->
            try {
              ...
              String hourStr = h.toString() // Use string key
              BigDecimal rate = e[4] as BigDecimal
              def room = index[r] ?: [:]
              def mode = room[m] ?: [:]
              def list = (mode[hourStr] ?: []) as List
              list << rate
...
def getAverageHourlyRate(String roomId, String hvacMode, Integer hour) {
  ...
  try {
    ...
    rates = hist?.hourlyRates?.get(roomId)?.get(hvacMode)?.get(hourStr) ?: []
  } catch (ignore) { rates = [] }
  ...
  if (totalCycleMinutes >= MIN_MINUTES_TO_SETPOINT) {
    ...
    data.ventIdsByRoomId.each { roomId, ventIds ->
      ...
      for (ventId in ventIds) {
        ...
        if (currentTemp == null) { // Handle null temp
            log(3, 'App', "Skipping rate calculation for ${roomName} (Temp unavailable)")
            continue
        }
        
        BigDecimal lastStartTemp = vent.currentValue('room-starting-temperature-c')
        if (lastStartTemp == null) {
             log(3, 'App', "Skipping rate calculation for ${roomName} (Start Temp unavailable)")
            continue
        }

        BigDecimal currentRate = vent.currentValue(ratePropName) ?: 0
        // Call the directional calculation function
        def newRate = calculateRoomChangeRate(lastStartTemp, currentTemp, totalCycleMinutes, percentOpen, currentRate, data.hvacMode)
  ...
  calcPercentOpen.each { ventId, percentOpen ->
    ...
    if (vent) {
      ...
      try {
        ...
        // Check if manually overridden before applying dampening
        boolean isOverridden = (atomicState?.manualOverrides ?: [:]).containsKey(ventId)
        def dampedTargetPercent = rawTargetPercent
        if (!isOverridden) {
            // Only apply dampening if not manually overridden
            dampedTargetPercent = applyChangeDampening(ventId, rawTargetPercent, hvacMode)
        }
  ...
  rateAndTempPerVentId.each { ventId, stateVal ->
    try {
      ...
      if (closeInactive && !stateVal.active) {
        ...
        // Unknown or too-low learning rate.
        // FIX: Check if we have a current position, if so, maintain it instead of resetting to 100%.
        def currentOpen = (vdev?.currentValue('percent-open') ?: vdev?.currentValue('level'))
        if (currentOpen != null && currentOpen > MIN_PERCENTAGE_OPEN) {
            percentageOpen = currentOpen
            log(3, 'App', "Maintaining current vent position (${percentageOpen}%) since change rate is unknown/low: ${stateVal.name}")
        } else {
            // Fallback to 100% only if we don't have a current position or it's already closed.
            percentageOpen = MAX_PERCENTAGE_OPEN
            log(3, 'App', "Opening vent at max since change rate is unknown/low and no previous position: ${stateVal.name}")
        }
      } else {
...
def calculateRoomChangeRate(def lastStartTemp, def currentTemp, def totalMinutes, def percentOpen, def currentRate) {
  ...
  try {
    // Call the main function with hvacMode=null (fallback to absolute difference)
...
def calculateRoomChangeRate(BigDecimal lastStartTemp, BigDecimal currentTemp, BigDecimal totalMinutes, int percentOpen, BigDecimal currentRate, String hvacMode = null) {
  // ... (Existing checks for MIN_MINUTES_TO_SETPOINT, MIN_RUNTIME_FOR_RATE_CALC, MIN_PERCENTAGE_OPEN remain the same) ...
  ...
  BigDecimal diffTemps = 0
  
  // Calculate directional temperature difference if hvacMode is provided
  if (hvacMode == COOLING) {
      diffTemps = lastStartTemp - currentTemp // Positive if cooling occurred
  } else if (hvacMode == HEATING) {
      diffTemps = currentTemp - lastStartTemp // Positive if heating occurred
  } else {
      // Fallback for tests or unknown mode: use absolute difference
      diffTemps = Math.abs(lastStartTemp - currentTemp)
  }

  // Check if temperature change is in the wrong direction or too small (if mode is known)
  if (hvacMode != null) {
      if (diffTemps < MIN_DETECTABLE_TEMP_CHANGE) {
        if (diffTemps < 0) {
            log(2, 'App', "Temperature moved in the wrong direction during ${hvacMode} (Start: ${lastStartTemp}, End: ${currentTemp}). Excluding from rate calculation.")
        } else {
            log(2, 'App', "Temperature change (${diffTemps}C) is below minimum detectable threshold (${MIN_DETECTABLE_TEMP_CHANGE}C).")
        }
        
        // If vent was significantly open but no positive change in the right direction, assign minimum efficiency
        if (percentOpen >= 30) {
          log(2, 'App', "Vent was ${percentOpen}% open but no meaningful improvement detected - assigning minimum efficiency")
          return MIN_TEMP_CHANGE_RATE
        }
        return -1
      }
  } else {
      // Original logic for absolute difference (fallback/test mode)
      if (Math.abs(diffTemps) < MIN_DETECTABLE_TEMP_CHANGE) {
        log(2, 'App', "Temperature change (${Math.abs(diffTemps)}C) is below minimum detectable threshold (${MIN_DETECTABLE_TEMP_CHANGE}C) - likely sensor noise")
  ...
  if (Math.abs(diffTemps) < TEMP_SENSOR_ACCURACY) {
    log(2, 'App', "Temperature change (${Math.abs(diffTemps)}C) is within sensor accuracy range (${TEMP_SENSOR_ACCURACY}C) - adjusting calculation")
    // Use a minimum reliable change for calculation to avoid division by near-zero
    diffTemps = diffTemps >= 0 ? Math.max(Math.abs(diffTemps), MIN_DETECTABLE_TEMP_CHANGE) : -Math.max(Math.abs(diffTemps), MIN_DETECTABLE_TEMP_CHANGE)
  }
  
  BigDecimal rate = Math.abs(diffTemps) / totalMinutes
  BigDecimal pOpen = percentOpen / 100

  // Use currentRate only if it's valid, otherwise use the newly calculated rate for normalization base
  BigDecimal normalizationBase = (currentRate != null && currentRate > 0) ? currentRate : rate
  
  // Ensure normalizationBase is not zero before division
  BigDecimal approxRate = (normalizationBase > 0) ? (rate / normalizationBase) / pOpen : 0
