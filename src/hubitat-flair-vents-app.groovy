...
@Field private Map __atomicStateFallback = [:]
@Field private boolean __useAtomicStateFallback = false
@Field private Map __settingsFallback = null
@Field private Object __locationFallback = null

Object getProperty(String name) {
  if (name == 'settings') {
    if (__settingsFallback instanceof Map) { return __settingsFallback }
    try { return super.getProperty(name) } catch (ignore) { return [:] }
  }
  if (name == 'location') {
    if (__locationFallback != null) { return __locationFallback }
    try { return super.getProperty(name) } catch (ignore) { return [timeZone: TimeZone.getTimeZone('UTC')] }
  }
  if (name == 'atomicState') {
    if (__useAtomicStateFallback) {
      if (!(__atomicStateFallback instanceof Map)) { __atomicStateFallback = [:] }
      return __atomicStateFallback
    }
    try {
      def inheritedAtomicState = super.getProperty(name)
      if (inheritedAtomicState instanceof Map) { return inheritedAtomicState }
    } catch (ignore) { }
    if (!(__atomicStateFallback instanceof Map)) { __atomicStateFallback = [:] }
    return __atomicStateFallback
  }
...
void setProperty(String name, Object value) {
  if (name == 'settings' && value instanceof Map) {
    __settingsFallback = (value as Map)
    mirrorAdvancedSettingsFromMap(__settingsFallback)
    return
  }
  if (name == 'location') {
    __locationFallback = value
    return
  }
  if (name == 'atomicState') {
    __useAtomicStateFallback = true
    __atomicStateFallback = (value instanceof Map) ? (value as Map) : [:]
    return
  }
  super.setProperty(name, value)
}

private void mirrorAdvancedSettingsFromMap(Map settingValues) {
  if (!(settingValues instanceof Map)) { return }
  try {
    if (settingValues.containsKey('dabHistoryRetentionDays')) {
      atomicState.dabHistoryRetentionDays = (settingValues.dabHistoryRetentionDays as Integer)
    }
    if (settingValues.containsKey('enableEwma')) {
      atomicState.enableEwma = (settingValues.enableEwma == true)
    }
    if (settingValues.containsKey('ewmaHalfLifeDays')) {
      atomicState.ewmaHalfLifeDays = (settingValues.ewmaHalfLifeDays as Integer)
    }
    if (settingValues.containsKey('enableOutlierRejection')) {
      atomicState.enableOutlierRejection = (settingValues.enableOutlierRejection == true)
    }
    if (settingValues.containsKey('outlierThresholdMad')) {
      atomicState.outlierThresholdMad = (settingValues.outlierThresholdMad as Integer)
    }
    if (settingValues.containsKey('outlierMode')) {
      atomicState.outlierMode = settingValues.outlierMode?.toString() ?: 'clip'
    }
    if (settingValues.containsKey('enableRawCache')) {
      atomicState.enableRawCache = (settingValues.enableRawCache == true)
    }
    if (settingValues.containsKey('rawDataRetentionHours')) {
      atomicState.rawDataRetentionHours = (settingValues.rawDataRetentionHours as Integer)
    }
  } catch (ignore) { }
}

private readOptionalSettingValue(String key, def defaultValue = null) {
  if (!key) { return defaultValue }
  try {
    if (__settingsFallback instanceof Map && __settingsFallback.containsKey(key)) {
      return __settingsFallback[key]
    }
  } catch (ignore) { }
  try {
    Map userMap = readUserSettingsMapForImport()
    if (userMap != null) {
      if (userMap.containsKey(key)) { return userMap[key] }
      return defaultValue
    }
  } catch (ignore2) { }
  try {
    def currentSettings = super.getProperty('settings')
    def value = currentSettings?."${key}"
...
private boolean readOptionalBooleanSetting(String key, boolean defaultValue = false) {
  def value = readOptionalSettingValue(key, defaultValue)
  if (value == null) { return defaultValue }
  if (value instanceof Boolean) { return value }
  String txt = value.toString().trim().toLowerCase()
  if (txt in ['true', '1', 'yes', 'on']) { return true }
  if (txt in ['false', '0', 'no', 'off', '']) { return false }
  return defaultValue
}

private Integer readOptionalIntegerSetting(String key, Integer defaultValue) {
  def value = readOptionalSettingValue(key, defaultValue)
  if (value == null) { return defaultValue }
  try { return (value as Integer) } catch (ignore) { return defaultValue }
}

private boolean isRawCacheEnabledSetting() {
  try {
    if (!isDabEnabled()) { return false }
  } catch (ignore) { return false }
  if (atomicState?.enableRawCache != null) {
    return (atomicState.enableRawCache == true)
  }
  return readOptionalBooleanSetting('enableRawCache', false)
}
...
@Field static final BigDecimal THERMOSTAT_HYSTERESIS = 0.6  // ~1 C equivalent
...
def mainPage() {
    ...
    if (state.flairAccessToken) {
      ...
      if (dabEnabled) {
        section('Thermostat & Globals') {
          ...
          if (settings.thermostat1AdditionalStandardVents != null && settings.thermostat1AdditionalStandardVents < 0) {
            app.updateSetting('thermostat1AdditionalStandardVents', 0)
          } else if (settings.thermostat1AdditionalStandardVents != null && settings.thermostat1AdditionalStandardVents > MAX_STANDARD_VENTS) {
  ...
  // Data smoothing and robustness (optional)
          section('DAB Data Smoothing (optional)') {
            ...
            input name: 'outlierThresholdMad', type: 'number', title: 'Outlier threshold (k sigma MAD)', defaultValue: 3, submitOnChange: true
          ...
          section {
            ...
            if (settings?.exportDabHistory) {
              handleExportDabHistory()
              app.updateSetting('exportDabHistory', '')
            }
...
def diagnosticsPage() {
  dynamicPage(name: 'diagnosticsPage', title: 'Diagnostics') {
    ...
    section('Raw Data Cache') {
      ...
      boolean rawCacheEnabled = isRawCacheEnabledSetting()
      def retentionHours = rawCacheEnabled ? getRawCacheRetentionHours() : RAW_CACHE_DEFAULT_HOURS
      paragraph "Raw cache enabled: ${rawCacheEnabled}"
      paragraph "Entries: ${entries.size()} | Retention (h): ${retentionHours}"
...
def resetCaches() {
  ...
  log(2, 'App', 'Instance caches cleared')
}
...
def initialize() {
  ...
  if (isDabEnabled()) {
    ...
    try {
      def thermostat = readOptionalSettingValue('thermostat1', null)
      if (thermostat) {
...
private BigDecimal getRoomTemp(def vent) {
  ...
  if (tempDevice) {
    ...
    if (readOptionalSettingValue('thermostat1TempUnit', '1')?.toString() == '2') {
...
private atomicStateUpdate(String stateKey, String key, value) {
  try {
    def root = atomicState
    if (!(root instanceof Map)) { return }
    def section = root[stateKey]
    if (!(section instanceof Map)) {
      section = [:]
      root[stateKey] = section
    }
    section[key] = value
    root[stateKey] = section
  } catch (ignore) { }
...
def getThermostatSetpoint(String hvacMode) {
  def thermostat = readOptionalSettingValue('thermostat1', null)
  if (thermostat == null) {
    try { thermostat = this.thermostat1 } catch (ignore) { }
  }
  if (thermostat == null) { return null }

  String thermostatUnit = readOptionalSettingValue('thermostat1TempUnit', '1')?.toString()
  BigDecimal setpoint
  ...
  if (setpoint == null) {
    ...
    logError 'Thermostat has no setpoint property'
    return null
  }
  if (thermostatUnit == '2') {
...
def cleanDecimalForJson(def value) {
  ...
  try {
    ...
    // Apply aggressive rounding to 9 decimal places
    def multiplier = 1000000000.0d  // 10^9 for 9 decimal places
    def rounded = Math.round(doubleValue * multiplier) / multiplier
...
// Zero-arg convenience overload used by tests and callers that only need duct-based detection.
def calculateHvacMode() { return calculateHvacModeRobust() }
...
def calculateHvacModeRobust() {
  def allDevices = getChildDevicesForImport() ?: []
  def vents = allDevices.findAll {
    readDeviceAttrForImport(it, 'duct-temperature-c') != null &&
    (readDeviceAttrForImport(it, 'room-current-temperature-c') != null ||
     readDeviceAttrForImport(it, 'current-temperature-c') != null ||
     readDeviceAttrForImport(it, 'temperature') != null)
  }
  ...
  def fallbackFromThermostat = {
    try {
      def thermostat = readOptionalSettingValue('thermostat1', null)
      if (thermostat == null) {
        try { thermostat = this.thermostat1 } catch (ignore0) { }
      }
      String op = thermostat?.currentValue('thermostatOperatingState')?.toString()?.toLowerCase()
  ...
  if (!allDevices || allDevices.isEmpty()) { return fallbackFromThermostat() }
  if (!vents || vents.isEmpty()) { return (fallbackFromThermostat() ?: 'idle') }

  def diffs = []
  vents.each { v ->
    try {
      BigDecimal duct = readDeviceAttrForImport(v, 'duct-temperature-c') as BigDecimal
      BigDecimal room = (readDeviceAttrForImport(v, 'room-current-temperature-c') ?:
                         readDeviceAttrForImport(v, 'current-temperature-c') ?:
                         readDeviceAttrForImport(v, 'temperature')) as BigDecimal
  ...
  // Use a stronger threshold for definitive calls to avoid false positives from borderline deltas.
  BigDecimal decisive = DUCT_TEMP_DIFF_THRESHOLD * 2
  if (median > decisive) { return HEATING }
  if (median < -decisive) { return COOLING }
  // Ambiguous moderate deltas: fall back only to thermostat state, otherwise unknown.
  if (median > DUCT_TEMP_DIFF_THRESHOLD || median < -DUCT_TEMP_DIFF_THRESHOLD) {
...
private Integer getRawCacheRetentionHours() {
  try {
    Integer h = readOptionalIntegerSetting('rawDataRetentionHours',
      (atomicState?.rawDataRetentionHours ?: RAW_CACHE_DEFAULT_HOURS) as Integer)
    if (h < 1) { h = 1 }
    if (h > 48) { h = 48 }
    return h
...
def sampleRawDabData() {
  if (!isRawCacheEnabledSetting()) { return }
  try {
    Long ts = now()
...
private initRequestTracking() {
  try {
    def stateMap = atomicState
    if (stateMap instanceof Map && stateMap.activeRequests == null) {
      stateMap.activeRequests = 0
    }
  } catch (ignore2) { }
}

private Map ensureFailureCounts() {
  try {
    def stateMap = atomicState
    if (!(stateMap instanceof Map)) { return [:] }
    if (!(stateMap.failureCounts instanceof Map)) {
      stateMap.failureCounts = [:]
    }
    return stateMap.failureCounts as Map
  } catch (ignore2) {
    return [:]
  }
}
...
def canMakeRequest() {
  initRequestTracking()
  Integer currentActiveRequests = 0
  try { currentActiveRequests = (atomicState?.activeRequests ?: 0) as Integer } catch (ignore) { currentActiveRequests = 0 }
  boolean strictLimitMode = false
  try {
    def st = atomicState
    strictLimitMode = (st instanceof Map && st.containsKey('lastRequestTime') && st.containsKey('requestCounts'))
  } catch (ignore2) { strictLimitMode = false }
  if (strictLimitMode) {
  ...
  if (currentActiveRequests >= MAX_CONCURRENT_REQUESTS) {
    ...
    try { atomicState.activeRequests = 0 } catch (ignore2) { }
...
def incrementActiveRequests() {
  initRequestTracking()
  try { atomicState.activeRequests = (atomicState?.activeRequests ?: 0) + 1 } catch (ignore) { }
}
...
def decrementActiveRequests() {
  initRequestTracking()
  def currentCount = 0
  try { currentCount = (atomicState?.activeRequests ?: 0) as Integer } catch (ignore) { currentCount = 0 }
  def newCount = Math.max(0, currentCount - 1)
  try { atomicState.activeRequests = newCount } catch (ignore2) { }
  log(1, 'App', "Decremented active requests from ${currentCount} to ${newCount}")
}
...
private void logError(String msg, String module = 'App', String correlationId = null) {
  int settingsLevel = (settings?.debugLevel as Integer) ?: 0
  try { log?.error "${msg}" } catch (ignore) { }
  if (settingsLevel > 0) {
    boolean __verbose = false
    try { __verbose = (atomicState?.verboseLogging == true) } catch (ignore4) { }
    if (__verbose) {
      def tz = location?.timeZone ?: TimeZone.getTimeZone('UTC')
      def entry = [ts: new Date().format("yyyy-MM-dd'T'HH:mm:ssZ", tz),
  ...
  def tz = location?.timeZone ?: TimeZone.getTimeZone('UTC')
  def ts = new Date().format('yyyy-MM-dd HH:mm:ss', tz)
...
def getDataAsync(String uri, String callback, data = null, int retryCount = 0) {
  def failureCounts = ensureFailureCounts()
  if (canMakeRequest()) {
    failureCounts.remove(uri)
...
def getDeviceDataWithCache(device, deviceId, deviceType, callback) {
  ...
  if (cachedData) {
    ...
    // Process cached data directly without going through handler (avoids extra decrementActiveRequests)
    if (callback == 'handlePuckGet' && cachedData?.data) {
      def puckData = cachedData.data
      if (puckData?.attributes?.'current-temperature-c' != null) {
        def tempC = puckData.attributes['current-temperature-c']
        def tempF = (tempC * 9.0/5.0) + 32
        sendEvent(device, [name: 'temperature', value: tempF, unit: 'F'])
      }
    }
    return
  }
...
def getDeviceReadingWithCache(device, deviceId, deviceType, callback) {
  ...
  if (cachedData) {
    ...
    // Process cached data directly to avoid calling handlers that would decrement active requests counter
    try {
      if (cachedData?.data?.attributes) {
        def attrs = cachedData.data.attributes
        if (callback == 'handlePuckReadingGet') {
          if (attrs.'room-temperature-c' != null) {
            def tempC = attrs['room-temperature-c']
            def tempF = (tempC * 9.0/5.0) + 32
            sendEvent(device, [name: 'temperature', value: tempF, unit: 'F'])
          }
          if (attrs.humidity != null) {
            sendEvent(device, [name: 'humidity', value: attrs.humidity, unit: '%'])
          }
        } else if (callback == 'handleDeviceGet') {
          processVentTraits(device, cachedData)
        }
      }
    } catch (e) {
      log(2, 'App', "Error processing cached reading for ${deviceId}: ${e?.message}")
    }
    return
  }
...
// NOTE: No decrementActiveRequests here - the base handlePuckGet handles it
...
// NOTE: No decrementActiveRequests here - the base handlePuckReadingGet handles it
...
def traitExtract(device, details, String propNameData, String propNameDriver = propNameData, unit = null) {
  try {
    ...
    logWarn err?.message ?: err.toString()
  }
}
...
def processVentTraits(device, details) {
  ...
  ['firmware-version-s', 'rssi', 'connected-gateway-name', 'created-at', 'duct-pressure',
   ...
   // Also update percent-open attribute directly (many app methods read it by name)
   if (details?.data?.attributes?.'percent-open' != null) {
     sendEvent(device, [name: 'percent-open', value: details.data.attributes['percent-open'], unit: '%'])
   }
...
def handleVentVerify(resp, data) {
  ...
  if (actual != target) {
    if (attempt < MAX_VENT_VERIFY_ATTEMPTS) {
      ...
      log(logLevel, 'App', "Vent ${device.getLabel()} reported ${actual}% instead of ${target}% (attempt ${attempt}/${MAX_VENT_VERIFY_ATTEMPTS}), retrying")
...
def thermostat1ChangeTemp(evt) {
  ...
  if (temp == null) {
    log(2, 'App', "Thermostat temperature is null, skipping")
    return
  }
  ...
  if (tempDiff >= THERMOSTAT_HYSTERESIS) {
    ...
    if (thermostatSetpoint != null && isThermostatAboutToChangeState(hvacMode, thermostatSetpoint, temp)) {
  ...
  ventIdsByRoomId.each { roomId, ventIds ->
    for (ventId in ventIds) {
      try {
        ...
        log(3, 'App', "Rebalancing Vents - ${vent.currentValue('room-name')} is at ${roomTemp} C (target: ${setPoint})")
...
private BigDecimal getEwmaRate(String roomId, String hvacMode, Integer hour) {
  try { return (atomicState?.dabEwma?.get(roomId)?.get(hvacMode)?.get(hour.toString())) as BigDecimal } catch (ignore) { return null }
}
...
private BigDecimal updateEwmaRate(String roomId, String hvacMode, Integer hour, BigDecimal newRate) {
  try {
    ...
    // Use String key since atomicState serializes map keys as Strings
    String hKey = hour.toString()
    BigDecimal prev = (mode[hKey]) as BigDecimal
    if (prev == null) {
      try {
        def hist = getHourlyRates(roomId, hvacMode, hour) ?: []
        if (hist && hist.size() > 0) {
          BigDecimal sum = 0.0
          hist.each { sum += (it as BigDecimal) }
          prev = sum / hist.size()
        }
      } catch (ignore0) { }
    }
    ...
    mode[hKey] = cleanDecimalForJson(updated)
    room[hvacMode] = mode
    map[roomId] = room
    try { atomicState.dabEwma = map } catch (ignore) { }
    return mode[hKey] as BigDecimal
...
private boolean isFanActive(String opState = null) {
  ...
  if (thermostat) {
    opState = opState ?: thermostat?.currentValue('thermostatOperatingState')
  ...
  if (totalCycleMinutes >= MIN_MINUTES_TO_SETPOINT) {
    ...
    data.ventIdsByRoomId.each { roomId, ventIds ->
      ...
      for (ventId in ventIds) {
        ...
        try {
          def seeded = (atomicState?.lastSeededRate ?: [:])?.get(roomId)?.get(data.hvacMode)?.get(hour.toString()) as BigDecimal
        ...
        if (persistedRate != null) {
          // appendHourlyRate writes to both hourlyRates index AND flat entries list
          // No need to also call appendDabHistory (which duplicates the flat entries write)
...
def initializeRoomStates(String hvacMode) {
  ...
  atomicState.ventsByRoomId.each { roomId, ventIds ->
    ...
    try {
      ...
      modeMap[currentHour.toString()] = (avgRate as BigDecimal)
...
def adjustVentOpeningsToEnsureMinimumAirflowTarget(rateAndTempPerVentId, String hvacMode, Map calculatedPercentOpen, additionalStandardVents) {
  ...
  // Guard against division by zero when all vents report the same temperature
  if (maxTemp == minTemp) { maxTemp = minTemp + 1.0 }
...
def calculateRoomChangeRate(BigDecimal lastStartTemp, BigDecimal currentTemp, BigDecimal totalMinutes, BigDecimal percentOpen, BigDecimal currentRate) {
  int percent = 0
  try { percent = (percentOpen ?: 0) as Integer } catch (ignore) { percent = 0 }
  return calculateRoomChangeRate(lastStartTemp, currentTemp, totalMinutes, percent, currentRate)
}
...
def handleExportDabHistory() {
  try {
    log(2, 'App', "Starting DAB history export")
...
def handleImportEfficiencyData() {
  try {
    ...
    def settingsMap = null
    try { settingsMap = getSettings() } catch (ignore) { }
    def rawImportInput = settingsMap?.get('importJsonData')
    def jsonData = normalizeImportInput(rawImportInput)
    if (!jsonData?.trim()) {
      def fromSettings = readOptionalSettingValue('importJsonData', null)
      def fromSettingsText = normalizeImportInput(fromSettings)
      if (fromSettingsText?.trim()) { jsonData = fromSettingsText }
    }
    if (!jsonData?.trim()) {
      try {
        def userMap = readUserSettingsMapForImport()
        def fallback = userMap?.settings?.importJsonData
        def fallbackText = normalizeImportInput(fallback)
        if (fallbackText?.trim()) { jsonData = fallbackText }
      } catch (ignore2) { }
    }
    if (!jsonData?.trim()) {
    ...
    if (result.success) {
      ...
      try {
        ...
        try { updateSetting('importJsonData', '') } catch (ignore4) { }
      }
...
def exportEfficiencyData() {
  ...
  getChildDevicesForImport().findAll { isVentDeviceForImport(it) || readDeviceAttrForImport(it, 'room-id') != null || readDeviceAttrForImport(it, 'room-name') != null }.each { device ->
    def coolingRate = readDeviceAttrForImport(device, 'room-cooling-rate') ?: 0
    def heatingRate = readDeviceAttrForImport(device, 'room-heating-rate') ?: 0

    def roomData = [
      roomId: readDeviceAttrForImport(device, 'room-id'),
      roomName: readDeviceAttrForImport(device, 'room-name'),
      ventId: readDeviceIdForImport(device),
...
def deactivateNightOverride() {
  try {
    ...
    if (settings?.nightOverrideRooms) {
      // Must read, modify, then reassign since atomicState uses JSON serialization
      def overrides = atomicState.manualOverrides ?: [:]
      settings.nightOverrideRooms.each { v ->
        def vent = (v instanceof String) ? getChildDevice(v) : v
        if (!vent) { return }
        String vid = vent.getDeviceNetworkId()
        overrides.remove(vid)
      }
      atomicState.manualOverrides = overrides
    } else {
...
def tileSetAutoMode(String tileDni) {
  try {
    String ventId = tileDni?.replaceFirst('^tile-','')
    if (atomicState?.manualOverrides) {
      def overrides = atomicState.manualOverrides ?: [:]
      overrides.remove(ventId)
      atomicState.manualOverrides = overrides
    }
    refreshVentTiles()
...
private String normalizeImportErrorMessage(String message) {
  def msg = (message ?: 'Unknown import error').trim()
  def lower = msg.toLowerCase()
  if (lower.contains('text must not be null or empty') || lower.contains('unexpected end of input')) {
    return 'Unexpected end of input'
  }
  if (lower.contains('unable to determine the current character') || lower.contains('unexpected character')) {
    return 'Unexpected character'
  }
  return msg
}

private boolean isVentDeviceForImport(device) {
  if (!device) { return false }
  try { return device.hasAttribute('percent-open') == true } catch (ignore) { }
  try {
    if (device instanceof Map && (device['hasAttribute'] instanceof Closure)) {
      return device['hasAttribute'].call('percent-open') == true
    }
  } catch (ignore2) { }
  return false
}
...
private String normalizeImportInput(rawValue) {
  ...
  try {
    if (rawValue?.toString()?.contains('UnvalidatedInput')) {
      ...
      def maybeName = rawValue?.name?.toString()
      if (maybeName && maybeName.trim() && maybeName != 'importJsonData') { return maybeName }
      return null
    }
  } catch (ignore) { }
  try {
    def maybeValue = rawValue?.value
    if (maybeValue != null) { return maybeValue.toString() }
  } catch (ignore3) { }
  try { return rawValue?.name?.toString() } catch (ignore2) { }
  return rawValue.toString()
}

private Map readUserSettingsMapForImport() {
  // Try property access first (safe in Hubitat sandbox)
  try {
    def value = this.hasProperty('userSettingsMap') ? this.userSettingsMap : null
    if (value instanceof Map) { return value as Map }
  } catch (ignore) { }
  // Fallback: try direct Groovy property access without reflection
  try {
    def value = this.metaClass?.getProperty(this, 'userSettingsMap')
    if (value instanceof Map) { return value as Map }
  } catch (ignore2) { }
  return null
}

private readDeviceAttrForImport(device, String attr) {
  if (!device) { return null }
  try { return device.currentValue(attr) } catch (ignore) { }
  try {
    if (device instanceof Map && (device['currentValue'] instanceof Closure)) {
      return device['currentValue'].call(attr)
    }
  } catch (ignore2) { }
  return null
}

private String readDeviceIdForImport(device) {
  if (!device) { return null }
  try { return device.getDeviceNetworkId()?.toString() } catch (ignore) { }
  try {
    if (device instanceof Map && (device['getDeviceNetworkId'] instanceof Closure)) {
      return device['getDeviceNetworkId'].call()?.toString()
    }
  } catch (ignore2) { }
  try { return device?.deviceNetworkId?.toString() } catch (ignore3) { }
  return null
}
...
def importEfficiencyData(jsonContent) {
  try {
    ...
    return [success: false, error: normalizeImportErrorMessage(e?.message)]
  }
}
...
def applyImportedEfficiencies(efficiencyData) {
  def childDevices = getChildDevicesForImport()
  def results = [
    globalUpdated: false,
    roomsUpdated: 0,
    roomsSkipped: 0,
  ...
  efficiencyData.roomEfficiencies?.each { roomData ->
    ...
    if (!device && roomData?.ventId != null) {
      def targetVentId = roomData.ventId.toString()
      device = childDevices.find { readDeviceIdForImport(it) == targetVentId }
    }
  ...
  if (efficiencyData.dabHistory) {
    ...
    log(2, 'App', "Restored DAB history (${efficiencyData.dabHistory.size()} rooms)")
  }

  if (efficiencyData.dabActivityLog) {
    atomicState.dabActivityLog = efficiencyData.dabActivityLog
    results.activityLogRestored = true
    log(2, 'App', "Restored DAB activity log (${efficiencyData.dabActivityLog.size()} entries)")
  }
...
def matchDeviceByRoomId(roomId) {
  ...
  def target = roomId.toString()
  return getChildDevicesForImport().find { device ->
    readDeviceAttrForImport(device, 'room-id')?.toString() == target
  }
}

def matchDeviceByRoomName(roomName) {
  if (roomName == null) { return null }
  def target = roomName.toString()
  return getChildDevicesForImport().find { device ->
    readDeviceAttrForImport(device, 'room-name')?.toString() == target
  }
}
  ...
  dynamicPage(name: 'dabProgressPage', title: 'DAB Progress', install: false, uninstall: false) {
    section {
      ...
      try {
        ...
        def nowRoom = hasVents ? settings?.progressRoom : null
...
String buildDabRatesTable() {
  ...
  vents.each { vent ->
    ...
    hours.each { hr ->
      ...
      if (hvacMode == 'both') {
        // Use string key since atomicState serializes map keys as strings
        def cooling = atomicState?.dabHistory?.hourlyRates?.get(roomId)?.get(COOLING)?.get(hr.toString()) ?: []
        def heating = atomicState?.dabHistory?.hourlyRates?.get(roomId)?.get(HEATING)?.get(hr.toString()) ?: []
...
String buildDabProgressTable() {
  ...
  Date start = null
  Date end = null
  try { if (settings?.progressStart instanceof String && settings.progressStart) { start = Date.parse('yyyy-MM-dd', settings.progressStart) } } catch (ignore) { }
  try { if (settings?.progressEnd instanceof String && settings.progressEnd) { end = Date.parse('yyyy-MM-dd', settings.progressEnd) } } catch (ignore) { }
...
// Legacy CI/test shim: handles GET callback style used by older tests.
def asyncHttpGetWrapper(resp, Map data = [:]) {
  try {
    String uri = data?.uri?.toString()
    boolean hasError = false
    try { hasError = (resp?.hasError() == true) } catch (ignore0) {
      try {
        if (resp instanceof Map && (resp['hasError'] instanceof Closure)) {
          hasError = (resp['hasError'].call() == true)
        }
      } catch (ignore1) { hasError = false }
    }
    if (hasError) {
      Integer status = 0
      try { status = (resp?.getStatus() ?: 0) as Integer } catch (ignore2) {
        try {
          if (resp instanceof Map && (resp['getStatus'] instanceof Closure)) {
            status = (resp['getStatus'].call() ?: 0) as Integer
          }
        } catch (ignore3) { status = 0 }
      }
      if (status >= 500) {
        incrementFailureCount(uri)
      }
    } else if (uri) {
      try { ensureFailureCounts().remove(uri) } catch (ignore4) { }
    }

    String callback = data?.callback ?: data?.originalCallback
    if (callback && this.metaClass.respondsTo(this, callback)) {
      this."$callback"(resp, data?.data)
    }
  } catch (ignore) { }
}
...
private String buildRawCacheJson() {
  try {
    ...
    def payload = [
      ...
      retentionHours: getRawCacheRetentionHours(),
      entries: trimmed
    ]
  ...
  dynamicPage(name: 'dabLiveDiagnosticsPage', title: 'DAB Live Diagnostics', install: false, uninstall: false) {
    section('Controls') {
      ...
      if (settings?.runDabDiagnostic) {
        try { runDabDiagnostic() } catch (e) { paragraph "Error: ${e?.message}" }
...
String renderDabDiagnosticResults() {
  ...
  sb << "<p><b>Global Setpoint:</b> ${results.inputs.globalSetpoint} &deg;C</p>"
  sb << '<h4>Room Data</h4>'
  sb << "<table border='1' style='width:100%'><tr><th>Room</th><th>Temp (&deg;C)</th><th>Rate</th></tr>"
  results.inputs.rooms.each { roomId, roomData ->
    sb << "<tr><td>${roomData.name}</td><td>${roomData.temp ?: '-'}</td><td>${roomData.rate ?: '-'}</td></tr>"
  }
  sb << '</table>'

  sb << '<h3>Calculations</h3>'
  sb << "<p><b>Longest Time to Target:</b> ${results.calculations.longestTimeToTarget} minutes</p>"
  ...
  results.calculations.initialVentPositions?.each { ventId, pos ->
    try { sb << "<tr><td>${ventId}</td><td>${roundToNearestMultiple(pos as BigDecimal)}%</td></tr>" } catch (ignore) { }
  }
  sb << '</table>'
...
def asyncHttpCallback(response, Map data) {
  try {
    ...
    // Unwrap the nested data so callbacks receive the original data map
    // (e.g., [device: deviceObj]) instead of the wrapper [originalCallback: ..., data: [...]]
    def callbackData = data?.data ?: data
    if (originalCallback && this.metaClass.respondsTo(this, originalCallback)) {
      // Dynamically call the original intended callback
      // NOTE: Individual callbacks handle their own decrementActiveRequests()
      // Do NOT decrement here to avoid double-decrement
      this."$originalCallback"(response, callbackData)
    } else {
      // No matching callback found - decrement here since no handler will
      decrementActiveRequests()
    }
  } catch (Exception e) {
